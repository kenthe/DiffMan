function [c] = lgdplog(dat,sh)
% LGDPLOG - Log of 'lgdirprod'-objects - Recursive function.
% function [c] = lgdplog(dat,sh)
...
if n == 1,           % Matrix Lie group data.
  ...
  elseif strcmp(sh{1},'lgsemprod'),
    ...
    elseif hasmatrix(obj),           % Matrix case.
      ...
      error('Don''t know how to calculate logarithm of this group');
    end;    
  end;
else,        % composite object
  c = cell(n,1); 
  for i = 1:n,
    c{i} = lgdplog(dat{i},sh{2}(i,:));
  end;
end;
return;
